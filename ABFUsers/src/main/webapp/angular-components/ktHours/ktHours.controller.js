webappApp.controller('ktHoursCtrl', [ '$scope', '$location', 'MasterDataService', 'toastr',
		'DataSetService', 'Session', 'ABF_CONSTANTS', ktHoursCtrl_fn ]);

function ktHoursCtrl_fn($scope, $location, MasterDataService, toastr, DataSetService, Session, ABF_CONSTANTS) {
	...
	$scope.fetchExistingResources = function(){
		try {
			if (angular.isObject($scope.contract) && $scope.contract.contractId > 0) {
				let url = "./kthours/fetchcontractkthours/"+$scope.contract.contractId;
	...
	$scope.openWeek = function (monthNm) {
		...
		$scope.week.weeklyTmpl = "./angular-components/ktHours/weeklyView.html";
	...
	$scope.resetResource = function (){
		...
		$scope.resource = {ktContractResourceId:0, contractId:-1, resourceType:null, businessLine:null, skill:null, band:null, role:null, grade:null, stayType:null, price:0, onShorePrice:-1, offShorePrice:-1, months:{month:[]}};		
	...
	$scope.bookHours = function(){
		if(angular.isArray($scope.resources) && $scope.resources.length > 0){
			MasterDataService.save('./kthours/create',$scope.resources)
			.then(function(response){
				var resData =  response.data;
	...
	$scope.deleteResource= function (item){
		...
		try{
			if(angular.isNumber(item.ktContractResourceId) && item.ktContractResourceId>0){
				MasterDataService.remove('./kthours/removektresource/', item.ktContractResourceId)
				.then(function(response){
					var resData =  response.data;
				...
				_.remove($scope.resources, function(currentObject) {
				    return currentObject.ktContractResourceId === item.ktContractResourceId;
				}); 
			}
