/*------------------------------------------------------------------------------
 *     Ident: Centre of Excellence Java
 *    Author: pawarraj
 * Copyright: (c) 10 august 2015 Sogeti Nederland B.V. All Rights Reserved.
 *------------------------------------------------------------------------------
 * Sogeti Nederland B.V.    |  No part of this file may be reproduced or 
 * Centre of Excellence Java|  transmitted in any form or by any means,        
 * Lange Dreef 17           |  electronic or mechanical, for the purpose,      
 * 4131 NJ VIANEN           |  without the express written permission of the   
 * The Netherlands          |  copyright holder.
 *------------------------------------------------------------------------------
 */

package com.sogeti.generic;

public final class CommonConstant
{

   private CommonConstant()
   {
   }

   public static final Integer maxUploadFileSize = 1048576;
   public static final Integer maxUploadImageSize = 536870;

   /**
    * for email notification
    */
   public static final String fromEmailId = "sogetinlresources@gmail.com";

   public static final String ccEmailId = "Anand.Basutkar@capgemini.com";

   public static final String emaiUserName = "sogetinlresources@gmail.com";

   public static final String emailPassword = "Sogeti@123";

   //----------End Email ------------------
   //Each record active or not
   public static final int RECORD_ACTIVE = 1;
   public static final int RECORD_INACTIVE = 2;
   
}
